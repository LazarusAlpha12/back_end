...
public class PersonaController {
    ...
    @GetMapping
    @PreAuthorize("hasRole('ADMINISTRADOR') or hasRole('ADMIN') or hasRole('OPERADOR_LOGISTICO')")
    public ResponseEntity<List<PersonaResponseDTO>> listarTodos() {
        return ResponseEntity.ok(personaService.listarTodos());
    }

    @GetMapping("/{id}")
    @PreAuthorize("hasRole('ADMINISTRADOR') or hasRole('ADMIN') or #id == authentication.principal")
